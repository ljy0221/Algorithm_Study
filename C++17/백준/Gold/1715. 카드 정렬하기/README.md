...
* **다익스트라(Dijkstra) 알고리즘**: 최단 경로 문제 해결에 사용
* **프림(Prim) 알고리즘**: 최소 신장 트리(MST) 구성

#### 문제 유형별 응용

* **병합 문제(Merge Problems)**: 카드 병합, 파일 병합 등
* **탐색 문제(Search Problems)**: 최적 경로 찾기, 상태 공간 탐색
* **이벤트 시뮬레이션(Event Simulation)**: 이벤트 기반 시뮬레이션
* **중간값 찾기(Median Finding)**: 두 개의 우선순위 큐를 사용
